...
module.exports = async (config: FullConfig) => {
  ...
  const { CUSTOMER_EMAIL, PASSWORD, ADMIN_USERNAME } = process.env;

  console.log(`Base URL: ${baseURL}`);

  // used across all playwright projects for authentication
  process.env.ADMIN_STATE = `${process.cwd()}/.state/admin-state.json`;
  process.env.CUSTOMER_STATE = `${process.cwd()}/.state/customer-state.json`;

  if (process.env.RUN_GLOBAL_SETUP === "1") {
    ...
    for (let i = 0; i < adminRetries; i++) {
      try {
        ...
        await adminLoginPage.enterUsername(ADMIN_USERNAME);
